...
const paymentMethodDeposit = asyncWrapper(async (req, res, next) => {
  ...
  if (!amount || isNaN(parsedAmount) || parsedAmount <= 0) {
    return next(
      AppError.create("Please provide a valid deposit amount.", 400, false),
    );
  }
  ...
  if (!finalMethodId) {
    ...
    if (userMethods.length === 0 || !userMethods[0].is_default) {
      return next(
        AppError.create(
          "Neither payment method provided nor default method found.",
          400,
          false,
        ),
      );
    ...
    if (!isOwner) {
      return next(
        AppError.create(
          "Invalid payment method or you do not have permission to use it.",
          403,
          false,
        ),
      );
  ...
  try {
    ...
    if (rowsUpdated === 0) {
      ...
      return next(
        AppError.create(
          "Failed to update wallet balance. Wallet may not exist.",
          500,
          false,
        ),
      );
