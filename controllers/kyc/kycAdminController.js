...
const listKyc = asyncWrapper(async (req, res, next) => {
    ...
    if (status && !allowedStatuses.includes(status)) {
        return next(
            AppError.create(`status filter must be one of: ${allowedStatuses.join(", ")}.`, 400, false)
        );
    }
...
const getKycById = asyncWrapper(async (req, res, next) => {
    ...
    if (!kyc) return next(AppError.create("KYC submission not found.", 404, false));
...
const reviewKyc = asyncWrapper(async (req, res, next) => {
    ...
    if (!action || !["approve", "reject"].includes(action)) 
        return next(AppError.create("action must be 'approve' or 'reject'.", 400, false));
    

    if (action === "reject" && !rejection_reason) 
        return next(AppError.create("rejection_reason is required when rejecting a submission.", 400, false));
    ...
    if (kyc.KYC_status === "approved") 
        return next(AppError.create("This submission has already been approved.", 400, false));
    
    if (kyc.KYC_status === "rejected" && action === "reject") 
        return next(AppError.create("This submission has already been rejected.", 400, false));
