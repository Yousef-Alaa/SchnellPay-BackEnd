...
const login = asyncWrapper(async (req, res, next) => {
    ...
    if (!email || !password) return next(AppError.create("All fields are required.", 400, false));

    
    const user = await UserModel.findByEmail(email);
    if (!user) return next(AppError.create("Invalid email or password.", 401, false));

    
    const isPasswordValid = await bcrypt.compare(password, user.password);
    if (!isPasswordValid) return next(AppError.create("Invalid email or password.", 401, false));
    

    if (!user.is_verified) {
        return next(
            AppError.create("Please verify your email before logging in.", 403, false)
        );
    }
